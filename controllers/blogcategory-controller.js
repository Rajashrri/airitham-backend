...
const now = new Date(); // ✅ Define now
    ...
    // ✅ Create new category
    const newCategory = await BlogCategory.create({ name, status, createdBy, url, createdAt, });
...
const getdatablogcategory = async (req, res) => {
  try {
...
const getblogcategoryByid = async (req, res) => {
  ...
  try {
    const response = await BlogCategory.find({ _id: id });
    if (!response) {
      res.status(404).json({ msg: "No Data Found" });
      return;
    }
...
const deleteblogcategory = async (req, res) => {
  try {
    ...
    // 1. Find the role
    const role = await BlogCategory.findById(id);
    if (!role) {
